...
export default function SummaryPage() {
  ...
  // ── 3. 시세: 평형대별·시설별 매매가/전세가 집계 (주거 시설, 전용면적 기준) ──
    ...
    <div className="max-w-6xl">
      ...
      {!hasAny ? (
        ...
        <>
          ...
          {/* ═══════════ 3. 시세 — 평형대별 매매가/전세가 비교 ═══════════ */}
          <SectionHead tone="violet" title="3. 시세 — 평형대별·시설별 매매가/전세가 비교"
            desc="시세 데이터만으로 주거 시설의 평형대별(전용면적 기준) 매매·전세 평균 평당가를 비교합니다. 매매는 매매 거래만(월세환산 제외), 전세는 보증금 기준입니다. 단위 천원/평." />
                    ...
                    {bandFacs.map((f) => (
                      <Fragment key={f}>
                        <tr className="border-t">
                          <td rowSpan={2} className="px-3 py-1.5 align-top font-medium">{f}</td>
