...
export function FacilityChart({ rows }: { rows: FacilityChartRow[] }) {
...
export function BuildingChart({ rows }: { rows: { name: string; avg: number }[] }) {
  ...
  const opt = {
    ...
    yAxis: { type: 'value' },
    series: [{ type: 'bar', data: data.map((r) => Math.round(r.avg)), itemStyle: { color: '#059669' }, barMaxWidth: 48, label: barLabel }],
  };
  return (
    <div className="rounded-lg border bg-white p-2">
